...
class DatabaseServiceImpl implements DatabaseService {
  ...
  DatabaseServiceImpl({ this.user });
  
  //region Init

  StorageService _storageService = StorageService();

  final CollectionReference _bookCollection = FirebaseFirestore.instance
      .collection('books');
  final CollectionReference _usersCollection = FirebaseFirestore.instance
      .collection('users');
  final CollectionReference _booksPerGenreCollection = FirebaseFirestore.instance
      .collection('booksPerGenre');
  final CollectionReference _forumDiscussionCollection = FirebaseFirestore.instance
  ...
  Future<void> initializeUser() {
    ...
    return _usersCollection.doc(user.uid).get().then((DocumentSnapshot doc) {
      if (!doc.exists) {
        _usersCollection.doc(user.uid).set(userMap)
  ...
  Future<bool> updateUserInfo(String imageProfilePath, bool oldImageRemoved, String fullName,
    ...
    updates['city'] = city;
    if (imageProfilePath != '') {
      Reference reference = await _storageService.addUserProfilePic(
          user.uid, imageProfilePath);
      String imgUrl = await _storageService.getUrlPicture(reference);
      updates['userProfileImageURL'] = imgUrl;
    } else if (oldImageRemoved) {
      await _storageService.removeUserProfilePic(user.uid);
      updates['userProfileImageURL'] = '';
    }

    _usersCollection.doc(user.uid).get().then((DocumentSnapshot doc) {
      if (doc.exists) {
        _usersCollection.doc(user.uid).update(
            updates
        )
  ...
  Stream<BookPerGenreMap> get perGenreBooks {
    Stream<BookPerGenreMap> result = _booksPerGenreCollection.snapshots()
        .map(_bookPerGenreListFromSnapshot);
    return result;
  }

  Stream<BookPerGenreUserMap> get userBooksPerGenre {
    Stream<BookPerGenreUserMap> result = _usersCollection.doc(user.uid)
        .snapshots()
  ...
  Stream<CustomUser> get userInfo {
    Stream<CustomUser> result = _usersCollection.doc(user.uid).snapshots()
  ...
  Stream<ForumDiscussion> get discussionInfo {
    Stream<ForumDiscussion> result = _forumDiscussionCollection
  ...
  Stream<Chat> get chatInfo {
    Stream<Chat> result = _chatsCollection
        .doc(_chat.chatKey)
        .snapshots()
  ...
  Stream<List<MyTransaction>> get transactionsInfo {
    Stream<List<MyTransaction>> result = _usersCollection
        .doc(user.uid)
        .snapshots()
  ...
  Future addUserBook(InsertedBook book) async {
    ...
    DocumentReference bookReference = _bookCollection.doc(generalInfoBookMap['id']);
    //DocumentReference sellerUserReference = _usersCollection.doc(sellingUser);
    ...
    await FirebaseFirestore.instance.runTransaction((transactionOnDb) async {
      if(!runningTransaction) {
        ...
        if (book.imagesPath != null) {
          /*List<String> imagesUrl = await __storageService.addBookPictures(
        ...
        book.imagesUrl = imagesUrl;*/
          ...
          for (int i = 0; i < book.imagesPath.length; i++) {
            Reference imgRef = await _storageService.addBookPicture(
                user.uid,
                book.title,
            ...
            String imgUrl = await _storageService.getUrlPicture(imgRef);
        ...
        DocumentReference booksPerGenre = _booksPerGenreCollection.doc(
            book.category);
  ...
  Future updateBook(InsertedBook book, int index, bool hadImages,
    ...
    await _storageService.removeBookPicture(
        user.uid, book.title, book.insertionNumber);

    /*await _usersCollection.doc(user.uid).get().then(
            (userDoc) {
          numberOfInsertedItems = userDoc.data()['numberOfInsertedItems'];
        });*/

    DocumentReference userBooksReference = _usersCollection.doc(user.uid);
    ...
    await FirebaseFirestore.instance.runTransaction((transactionOnDb) async {
      if(!runningTransaction) {
        ...
        DocumentReference bookReference = _bookCollection.doc(books[index]['id']);

        if (book.imagesPath != null) {
          /*List<String> imagesUrl = await _storageService.addBookPictures(
            user.uid, book.title, numberOfInsertedItems, book.images);
        book.imagesUrl = imagesUrl;*/
          List<String> imagesUrl = List<String>();
          for (int i = 0; i < book.imagesPath.length; i++) {
  ...
  Future<BookPerGenreUserMap> getUserBooksPerGenreSnapshot() async {
    BookPerGenreUserMap result = await _usersCollection.doc(user.uid)
        .snapshots()
        .map(_bookPerGenreUserListFromSnapshot)
        .elementAt(0);
    return result;
  ...
  Future<CustomUser> getUserSnapshot() async {
    CustomUser result = await _usersCollection.doc(user.uid).snapshots()
        .map(_userInfoFromSnapshot).elementAt(0);
    return result;
  }

  Future<CustomUser> getUserById(String uid) async {
    CustomUser result = await _usersCollection.doc(uid).snapshots()
        .map(_userInfoFromSnapshot).elementAt(0);
    return result;
  }
  ...
  Future removeBook(int index, InsertedBook book) async {
    ...
    DocumentReference userReference = _usersCollection.doc(user.uid);
    await FirebaseFirestore.instance.runTransaction((transactionOnDb) async {
      if(!runningTransaction) {
        runningTransaction = true;
        WriteBatch batch = FirebaseFirestore.instance.batch();
        ...
        DocumentReference bookReference = _bookCollection.doc(id);
        ...
        if (bookDocumentRemoved) {
          ...
          DocumentReference booksPerGenreReference = _booksPerGenreCollection
              .doc(book.category);
  ...
  Future<dynamic> getGeneralBookInfo(String bookId) async {
    dynamic book;
    await _bookCollection.doc(bookId).get().then((value) {
      book = value.data();
    });
    return book;
  }
  ...
  Future<dynamic> getBookSoldBy(String bookId) async {
    ...
    await _bookCollection.doc(bookId).get().then((valueBook) async {
      ...
      for (int i = 0; i < valueBook.data()['owners'].length; i++) {
        ...
        await _usersCollection.doc(own).get().then((valueUser) {
  ...
  Future<dynamic> getMyFavoriteBooks() async {
    ...
    await _usersCollection.doc(user.uid).get().then((userDoc) async {
      likedBooks = userDoc.data()['booksILike'];
      for (int i = 0; i < likedBooks.length; i++) {
        await _usersCollection.doc(likedBooks[i]['userUid']).get().then((userDoc) {
  ...
  Future<dynamic> getMyOrders() async {
    ...
    await _usersCollection.doc(user.uid).get().then((userDoc) async {
      ...
      for (int i = 0; i < transactionsAsBuyer.length; i++) {
        await _transactionsCollection.doc(transactionsAsBuyer[i]).get().then((userDoc) {
  ...
  Future<bool> canIReview(String toReviewUid) async {
    ...
    await _usersCollection.doc(user.uid).get().then((DocumentSnapshot doc) {
      if(doc.exists) {
    ...
    if(myTransactionsAsBuyer != null) {
      for(int i = 0; !result && i < myTransactionsAsBuyer.length; i++) {
        await _transactionsCollection.doc(myTransactionsAsBuyer[i]).get().then((DocumentSnapshot doc) {
  ...
  Future<void> followUser(CustomUser followed) async {
    ...
    await _usersCollection.doc(followed.uid)
        .update({
  ...
  Future<void> addLike(int bookInsertionNumber, String thumbnail, String userWhoLikes) async {
    ...
    await _usersCollection.doc(Utils.mySelf.uid).update({
  ...
  Future<void> addReview(ReceivedReview review) async {
    ...
    await _usersCollection.doc(reviewerUid)
        .update({
  ...
  removeReviews(List<ReviewWrittenByMe> reviewsToDelete) async {
    ...
    await _usersCollection.where('uid', whereIn: reviewedUsersUids).get().then(
          ...
          querySnapshot.docs.forEach((element) {
            ...
            _usersCollection.doc(element['uid']).update({
              'receivedReviews': receivedReviews
            });
          });
        }
  ...
  Future<dynamic> getForumDiscussions() async {
    QuerySnapshot snapshot = await _forumDiscussionCollection.get();
  ...
  Future<dynamic> removeDiscussion(String title) async {
    print("Deleting");
    await _forumDiscussionCollection.doc(title).delete();
  }

  Future<dynamic> createNewDiscussion(String category, String title) async {
    dynamic result;

    await _forumDiscussionCollection.doc(title).get().then((DocumentSnapshot doc) async {
      if (!doc.exists) {
        ForumDiscussion newDiscussion = ForumDiscussion(
            title, category, List<Message>(), DateTime.now(), user.uid
        );
        newDiscussion.setKey();
        await _forumDiscussionCollection.doc(title).set(newDiscussion.toMap())
  ...
  Future<List<Message>> addMessageToForum(String message, ForumDiscussion discussion, CustomUser userFromDb) async {
    ...
    await _forumDiscussionCollection.doc(discussion.title).get().then((DocumentSnapshot doc) async {
      if (doc.exists) {
    ...
    await _forumDiscussionCollection.doc(discussion.title).update(
  ...
  Future purchaseAndProposeExchange(String sellingUser, chosenShippingMode, shippingAddress, payCash, List<InsertedBook> booksToPurchase, Map<InsertedBook, Map<String, dynamic>> booksToExchange, Map<int, String> thumbnails) async {
    ...
    await _usersCollection.doc(sellingUser).get().then((DocumentSnapshot doc) {
      if(doc.exists) {
    ...
    await FirebaseFirestore.instance.runTransaction((transactionOnDb) async {
      ...
      for (int i = 0; i < booksToRemoveIdNoDuplicates.length; i++){
        DocumentReference bookRef = _bookCollection.doc(booksToRemoveIdNoDuplicates[i]);
        ...
        if (bookData['availableNum'] == 0) {
          ...
          for (int j = 0; j < bookData['owners'].length && removeBookFromBooksPerGenre; j++){
            List<String> owners = List.from(bookData['owners']);
            print(owners[j]);
            DocumentReference bookRef = _usersCollection.doc(owners[j]);
            DocumentSnapshot bookSnap = await transactionOnDb.get(bookRef);
      ...
      for(int i = 0; i < booksToRemoveFromBooksPerGenres.length; i++){
        DocumentReference documentReference = _booksPerGenreCollection.doc(genres[i]);
      ...
      for (int i = 0; i < booksFromBooksCollectionToDelete.length; i++){
        DocumentReference documentReference = _bookCollection.doc(booksFromBooksCollectionToDelete[i]);
      ...
      for (int i = 0; i < booksToRemoveIdNoDuplicates.length; i++){
        if (booksFromBooksCollectionToModify[booksToRemoveIdNoDuplicates[i]] != null){
          DocumentReference documentReference = _bookCollection.doc(booksToRemoveIdNoDuplicates[i]);
    ...
    if (transactionSuccessfullyCompleted) {
      print('Step 7');
      await _transactionsCollection.doc(transaction['id']).set(transaction)
          .catchError((error) => errorMessage = "Failed to add transaction");

      if (errorMessage == null) {
        print('Step 8');
        await _usersCollection.doc(transaction['seller']).update({
  ...
  Future acceptExchange(String transactionId, String seller, Map<String, dynamic> sellerBook,
    ...
    await FirebaseFirestore.instance.runTransaction((transactionOnDb) async {
      ...
      if (booksToRemoveIdNoDuplicates.length == 1) {
        DocumentReference bookRef = _bookCollection.doc(
  ...
  Future<dynamic> getTransactionFromKey(String transactionKey) async {
    dynamic result;
    await _transactionsCollection.doc(transactionKey).get().then((DocumentSnapshot doc) {
      if(doc.exists) {
        result = doc.data();
      }
    });
  ...
  Future<dynamic> getMyChats() async {
    ...
    for(int i = 0; i < chatKeys.length; i++) {
      await _chatsCollection.doc(chatKeys[i]).get().then((DocumentSnapshot doc) {
        if(doc.exists) {
          result.add(doc.data());
        }
      });
  ...
  Future<dynamic> createNewChat(String userUid1, String userUid2, String userUid1Username, String userUid2Username) async {
    ...
    await _chatsCollection.doc(chatKey).get().then((DocumentSnapshot doc) async {
      if (!doc.exists) {
        Chat newChat = Chat(
            userUid1, userUid2, userUid1Username, userUid2Username, List<Message>(), time
        );
        newChat.setKnownKey(chatKey);
        await _chatsCollection.doc(chatKey).set(newChat.toMap())
  ...
  Future<List<Message>> addMessageToChat(String message, Chat chat, CustomUser userFromDb) async {
    dynamic currentMessages;
    await _chatsCollection.doc(chat.chatKey).get().then((DocumentSnapshot doc) async {
      if (doc.exists) {
        currentMessages = doc.get("messages");
      }
    });
    ...
    newMessage.setKey();
    currentMessages.add(newMessage.toMap());
    await _chatsCollection.doc(chat.chatKey).update(
        {"messages" : currentMessages}
    );
